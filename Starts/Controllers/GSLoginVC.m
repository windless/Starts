...
- (void)viewWillAppear:(BOOL)animated {
  dispatch_after(
      ...
      dispatch_get_main_queue(), ^{ [self showLoginForm]; });
}

#pragma mark layout

- (void)layoutLogoLabel {
  ...
  [label gsl_horizontalCenterIn:self.view];
...
- (void)layoutLoginForm {
  ...
  loginForm.frame = CGRectInset(self.view.frame, 30, 120);
  loginForm.alpha = 0;
...
- (void)layoutProfileView {
  ...
  loadingView.alpha = 0;
  [loadingView gsl_horizontalCenterIn:self.view];
...
- (void)login {
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)),
                 dispatch_get_main_queue(), ^{ [self showProfile]; });
}
